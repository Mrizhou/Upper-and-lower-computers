...
class Main_ui:
    ...
    def handle_data(self, data):
        ...
        # 保存GPS数据到日志
        logger.info(
            f"Latitude: {self.data_recv['latitude']}, Longitude: {self.data_recv['longitude']}"
        )
        logger.handlers[0].flush()  # 确保日志内容被刷新到文件
    ...
    def on_timeout1(self):
        ...
        if hasattr(self, "tas2") and self.tas2 and self.task_obj:
            ...
            if delta_x == 0 and delta_y == 0:
                ...
                self.task_obj["compass"] = angle_string_to_float(
                    self.data_recv["compass"]
                )
