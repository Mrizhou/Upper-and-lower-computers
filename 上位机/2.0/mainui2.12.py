...
from headingholdtask2_6 import headingholdtask
...
class MainUI:
    ...
    def model_choose(self):
        ...
        elif self.ui.MODEL_2.currentText() == "路径跟踪2":
            self.path_points.clear()
            self.current_path_index = 0
            self.ui.GPS_task.setRowCount(0)
            self.ui.GPS_task.setHorizontalHeaderLabels([])
            ...
            with open("task.txt", "r") as file:
                for line in file:
                    try:
                        ...
                        self.path_points.append((longitude, latitude))
                        self.add_row_to_table(longitude, latitude)
                    except ValueError:
                        print(f"无法解析行: {line.strip()}")
            self.task_obj["mode"] = "路径跟踪2"
            self.tas3 = headingholdtask()
            self.tas3.data_send.connect(self.task_normal)
    ...
    def add_row_to_table(self, longitude, latitude):
        row_position = self.ui.GPS_task.rowCount()
        self.ui.GPS_task.insertRow(row_position)
        self.ui.GPS_task.setItem(row_position, 0, QTableWidgetItem(str(longitude)))
        self.ui.GPS_task.setItem(row_position, 1, QTableWidgetItem(str(latitude)))
    ...
    def task_normal(self, data):
        if hasattr(self, "dov") and self.tas:
            self.data_send["Rsend"] = max(4.9, min(data["Rsend"] * self.gear + 7.4, 9.8))
            self.data_send["Lsend"] = max(4.9, min(data["Rsend"] * self.gear + 7.4, 9.8))
        if hasattr(self, "tas3") and self.tas3 and self.task_obj and len(self.path_points) >= 2:
            self.current_path_index = data["current_path_index"]
            self.data_send["Rsend"] = max(4.9, min(data["Rsend"] + 7, 9.8))
            self.data_send["Lsend"] = max(4.9, min(data["Lsend"] + 7, 9.8))
        if (hasattr(self, "tas2") and self.tas2) or (hasattr(self, "tas") and self.tas):
            self.data_send["Rsend"] = max(4.9, min(data["Rsend"] + 7, 9.8))
            self.data_send["Lsend"] = max(4.9, min(data["Lsend"] + 7, 9.8))
