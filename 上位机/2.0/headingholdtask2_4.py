...
class headingholdtask(QThread):
    ...
    def track_track(self):
        ...
        # 计算直线起点到终点的距离和方位角
        az12, _, _ = geod.inv(
            self.current_data["start_longitude"],
            self.current_data["start_latitude"],
            self.current_data["end_longitude"],
            self.current_data["end_latitude"],
        )

        # 计算直线起点到目标点的距离和方位角
        az1t, _, dist1t = geod.inv(
            self.current_data["start_longitude"],
            self.current_data["start_latitude"],
            self.current_data["longitude"],
            self.current_data["latitude"],
    ...
    def on_update_data(self, new_data):
        ...
        if self.current_data["mode"] == "航向保持":
            ...
            self.current_data["start_longitude"] = new_data["start_longitude"]
            self.current_data["start_latitude"] = new_data["start_latitude"]
            self.current_data["end_longitude"] = new_data["end_longitude"]
            self.current_data["end_latitude"] = new_data["end_latitude"]
