...
class headingholdtask(QThread):
    ...
    def track_target(self):
        ...
        if distance < self.stop_distance:
            ...
            if distance < self.slowdown_distance:
                ...
                scale_factor = 1  # 正常速度
            self.data["Lsend"] = round(ship_control_data[0] / 180 * scale_factor * 3)
            self.data["Rsend"] = round(ship_control_data[1] / 180 * scale_factor * 3)
    ...
    def hold_heading(self):
        ...
        ship_control_data = ship.heading_control()
        self.data["Lsend"] = round(ship_control_data[0] / 180 * 3)
        self.data["Rsend"] = round(ship_control_data[1] / 180 * 3)
        # 更新Es
        self.previous_E = ship.E  # 更新上一时刻的E
    ...
    def track_track(self):
        ...
        if dist2t < self.stop_distance:
            ...
            if dist2t < self.slowdown_distance:
                scale_factor = max(
                    0.1,
                    (dist2t - self.stop_distance) / (self.slowdown_distance - self.stop_distance),
                )  # 距离越小，速度越慢，最小为0.1倍速
            else:
                scale_factor = 1  # 正常速度
            self.data["Lsend"] = round(ship_control_data[0] / 180 * scale_factor * 3)
...
class ShipSimulation:
    ...
    def heading_control(self):
        ...
        self.Es = self.E
        self.Lpwm = self.zengliang
        self.Rpwm = -self.zengliang
        if self.Lpwm > 180:
            self.Lpwm = 180
        if self.Lpwm < -180:
            self.Lpwm = -180
